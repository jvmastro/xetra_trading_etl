"""Integration test XetraETL Methods"""
...
class IntTestXetraETLMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s3_bucket_name_src = 'xetra-int-test-src-jm'
        self.s3_bucket_name_trg = 'xetra-int-test-trg-jm'
        ...
        # Creating date list for testing
        ...
        data = [
            ['AT0000A0E9W5', 'SANT', self.dates[5],
            ...
            ['AT0000A0E9W5', 'SANT', self.dates[1], 
                '09:00', 24.22, 22.21, 22.21, 25.01, 1523]]
        self.df_src = pd.DataFrame(data, columns=columns_src)
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[0:0], f'{self.dates[5]}/{self.dates[5]}_BINS_XETR12.csv', 'csv')
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[1:1], f'{self.dates[4]}/{self.dates[4]}_BINS_XETR13.csv', 'csv')
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[2:2], f'{self.dates[3]}/{self.dates[3]}_BINS_XETR14.csv', 'csv')
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[3:3], f'{self.dates[3]}/{self.dates[3]}_BINS_XETR15.csv', 'csv')
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[4:4], f'{self.dates[2]}/{self.dates[2]}_BINS_XETR16.csv', 'csv')
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[5:5], f'{self.dates[2]}/{self.dates[2]}_BINS_XETR17.csv', 'csv')
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[6:6], f'{self.dates[1]}/{self.dates[1]}_BINS_XETR07.csv', 'csv')
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[7:7], f'{self.dates[1]}/{self.dates[1]}_BINS_XETR08.csv', 'csv')
        self.s3_src_bucket.write_df_to_s3(
            self.df_src.loc[8:8], f'{self.dates[1]}/{self.dates[1]}_BINS_XETR09.csv', 'csv')
    ...
    def test_int_etl_report1_no_metafile(self):
        ...
        meta_exp = [self.dates[3], self.dates[2], self.dates[1]]
